...
export async function createOrder({ items }: { items: CartItem[] }) {
  ...
  if (!userId) {
    console.error(
      "Usuario no autenticado - session no encontrada o cookie faltante"
    );
    throw new Error("Usuario no autenticado. Verifique su sesión.");
  }

  const connAny = mongoose.connection as any;
  const supportsTransactions = !!(
    connAny.client &&
    connAny.client.topology &&
    connAny.client.topology.s &&
    (connAny.client.topology.s.coreTopology ||
      connAny.client.topology.s.replicaSet)
  );
